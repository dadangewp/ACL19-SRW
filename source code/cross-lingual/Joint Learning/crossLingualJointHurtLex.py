...
def hurtLexEmbedding1 (X):
    embeddings_index = {}
    tokenizer = Tokenizer(num_words = 15000)
    tokenizer.fit_on_texts(X)
    with codecs.open('es-hurtlex-embedding.vec', encoding='ANSI') as f:
...
def hurtLexEmbedding2 (X):
    embeddings_index = {}
    tokenizer = Tokenizer(num_words = 15000)
    tokenizer.fit_on_texts(X)
    with codecs.open('it-hurtlex-embedding.vec', encoding='ANSI') as f:
        print("read embedding...")
        for line in f:
            values = line.split("\t")
            word = values[0]
...
def buildEmbedding2 (X):
    ...
    tokenizer = Tokenizer(num_words = 15000)
    tokenizer.fit_on_texts(X)
    with codecs.open('wiki.multi.it.vec', encoding='ANSI') as f:
        print("read embedding...")
        for line in f:
            values = line.split(" ")
            word = values[0]
...
def RNN(X,X2):
    ...
    vocab3, hurtlex_matrix1 = hurtLexEmbedding1(X)
    vocab4, hurtlex_matrix2 = hurtLexEmbedding2(X2)
    ...
    en_hurtlex = Embedding(vocab3+1, 17, input_length=100, weights=[hurtlex_matrix1], trainable=False)(en_inputs)
    es_hurtlex = Embedding(vocab4+1, 17, input_length=100, weights=[hurtlex_matrix2], trainable=False)(es_inputs)
    lstm1 = LSTM(16)(en_embedding)
    lstm2 = LSTM(16)(es_embedding)
    lstm3 = LSTM(2)(en_hurtlex)
    lstm4 = LSTM(2)(es_hurtlex)
    concat1 = concatenate([lstm1, lstm3], axis=-1)
    concat2 = concatenate([lstm2, lstm4], axis=-1)
    dense1 = Dense(4,name='FC1')(concat1)
    dense1 = Activation('relu')(dense1)
    dense1 = Dropout(0.4)(dense1)
    dense2 = Dense(4,name='FC2')(concat2)
...
dataTrain, dataLabel = parse_testing("it-es-evalita-training.txt")
dataTrain2, dataLabel2 = parse_testing("it_training.tsv")

dataTest, labelTest = parse_testing("test_es.tsv")  
dataTest2, labelTest2 = parse_testing("it-hateval-testing.txt") 
...
model.fit(allxtrain,Y_train,batch_size=16,epochs=3,
